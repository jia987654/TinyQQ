...
void net::Server::start()
{
	...
	// 初始化监听器
	m_acceptor = new Acceptor(m_loop.get(), &m_sock, m_addr);
...
void net::Server::newConnect(int cfd, IPAddress* addr)
{
	...
	conn->setCloseCallback(std::bind(&Server::removeConnect, this, std::placeholders::_1));
...
{
	if (m_closeCallback) m_closeCallback(conn);
